...
class Tools {
	...
	def loadProperties(OptionAccessor opts) {
	   ...
	   println("set command line arguments")
		...
		properties.load(new File("${getScriptDir()}/DB2Bind.properties"))
		...
		properties.load(new File("${getScriptDir()}/FileTypeMap.properties"))

		// load bindlinkEditScanner.properties containing Link Edit scanning options used by Mortgage Application build
		properties.load(new File("${getScriptDir()}/Global.properties"))
