pipeline {
    ...
    stages {
        ...
        stage("Test") {
            ...
            steps {
                sh "${env.groovyHome}/groovy --classpath .:${env.javaClassPath}:$WORKSPACE/${env.polycephalyJar} $WORKSPACE/build/build.groovy --application Polycephaly --sourceDir $WORKSPACE/conf/package.txt"
            }
        }
        stage("Deploy") {
            options {
